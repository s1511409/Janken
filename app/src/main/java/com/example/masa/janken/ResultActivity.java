...
public class ResultActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // コンピュータの手を決める
        int comHand = getHand();
        ImageView comHandImageView = (ImageView) findViewById(R.id.com_hand_imag);
        switch (comHand) {
            case JANKEN_GU:
    ...
    private void saveData(int myHand, int comHand, int gameResult) {
        ...
        int winningSteakCount = pref.getInt("WINNING_STEAK_COUNT", 0);
        ...
        if (lastGameResult == 2 && gameResult == 2) {
            editor.putInt("WINNING_STEAK_COUNT", winningSteakCount + 1);
        } else {
            editor.putInt("WINNING_STEAK_COUNT", 0);
        }
        editor.putInt("LAST_MY_HAND", myHand);
        editor.putInt("LAST_COM_HAND", comHand);
        editor.putInt("BEFORE_LAST_COM_HAND", lastComHand);
        editor.putInt("GAME_REDULT", gameCount);
